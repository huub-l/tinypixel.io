...
class Total_Amount extends Custom_Field {
	...
	public static function print_total_amount_label( $settings ) {
		...
		$html .= $label . ' <span class="simpay-total-amount-value">' . simpay_formatted_amount( self::$total_amount + simpay_calculate_tax() ) . '</span>';
		$html .= '</p>';
		$html .= '</div>';
	...
	public static function print_recurring_total_label( $settings ) {
		...
		if ( $simpay_form->subscription_custom_amount && $simpay_form->subscription_interval > 1 ) {
			$amount_text = '<span class="simpay-total-amount-recurring-value">' . simpay_formatted_amount( self::$recurring_total + simpay_calculate_tax( self::$recurring_total ) ) . ' every ' . $simpay_form->subscription_interval . ' ' . $simpay_form->subscription_frequency . 's</span>';
		} else {
			$amount_text = '<span class="simpay-total-amount-recurring-value">' . simpay_formatted_amount( self::$recurring_total + simpay_calculate_tax( self::$recurring_total ) ) . '/' . $simpay_form->subscription_frequency . '</span>';
		}
	...
	// TODO Need these public properties & functions?
