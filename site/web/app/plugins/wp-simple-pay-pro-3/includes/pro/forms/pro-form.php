...
class Pro_Form extends Default_Form {
	...
	public function print_custom_amount( $print_wrapper = true ) {
		...
		if ( empty( $default_value ) || ( $default_value < $min ) ) {
			$default_value = '';
		}
		...
		$html .= '<input id="' . $field_id . '" name="' . esc_attr( $input_name ) . '" data-error="#simpay-custom-amount-input-' . esc_attr( $this->id ) . '-error" class="simpay-custom-amount-input simpay-amount-input" type="' . esc_attr( $custom_amount_input_type ) . '" value="' . esc_attr( simpay_formatted_amount( $default_value, '', false ) ) . '" />';
	...
	public function pro_print_custom_fields( $html, $item ) {
		...
		// Always start over with custom field html to return.
		$html = '';
		...
		switch ( $item['type'] ) {
			...
			case 'plan_select':
				...
				if ( $this->is_subscription() ) {
					...
					$amount = $this->recurring_amount_to_tax();

					Fields\Total_Amount::set_recurring_total( $amount );
					$html = $this->print_subscription_options( $this->subscription_custom_amount );
				}
			...
			case 'total_amount':
				...
				$amount = $this->initial_amount_to_tax();

				Fields\Total_Amount::set_total( $amount );
		...
		return apply_filters( 'simpay_custom_field_html', $html, $item );
	}

	/**
	 * Total initial amount to apply tax to.
	 *
	 * Use minimum amount for first tax & total amount calculations when using custom amount.
	 * Check for subscription custom amount (set single plan) before one-time custom amount.
	 * Default amount is optional and is calculated into tax & total amounts client-side after page load.
	 * Also caluclate subscription trials and/or setup fees.
	 */
	public function initial_amount_to_tax() {

		if ( $this->is_trial ) {
			$amount = 0;
		} elseif ( $this->subscription_custom_amount && empty( $this->default_plan ) ) {
			$amount = floatval( $this->subscription_minimum_amount );
		} elseif ( $this->one_time_custom_amount ) {
			$amount = floatval( $this->minimum_amount );
		} else {
			$amount = floatval( $this->amount );
		}

		// Individual plan setup fees (multi-plan fees) are calculated client-side currently.
		if ( $this->is_subscription() ) {
			$amount += floatval( $this->subscription_setup_fee );
		}
	...
	/**
	 * Recurring amount to apply tax to.
	 */
	public function recurring_amount_to_tax() {

		if ( ! $this->is_subscription() ) {
			return 0;
		}

		if ( $this->subscription_custom_amount && empty( $this->default_plan ) ) {
			$amount = floatval( $this->subscription_minimum_amount );
		} else {
			$amount = floatval( $this->amount );
		}
	...
	public function pro_set_post_meta_settings() {
		...
		/* custom amount options */
		$this->minimum_amount      = simpay_convert_amount_to_cents( simpay_get_filtered( 'minimum_amount', simpay_get_saved_meta( $this->id, '_minimum_amount', '100' ), $this->id ) );
		$this->default_amount      = simpay_convert_amount_to_cents( simpay_get_filtered( '_default_amount', simpay_get_saved_meta( $this->id, '_custom_amount_default' ), $this->id ) );
		...
		$this->subscription_minimum_amount      = simpay_convert_amount_to_cents( simpay_get_filtered( 'subscription_minimum_amount', simpay_get_saved_meta( $this->id, '_multi_plan_minimum_amount' ), $this->id ) );
		$this->subscription_default_amount      = simpay_convert_amount_to_cents( simpay_get_filtered( 'subscription_default_amount', simpay_get_saved_meta( $this->id, '_multi_plan_default_amount' ) ) );
		...
		// Note these are applies PRE-TAX and for one-time amounts only.
