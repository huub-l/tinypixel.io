...
class Stripe_API {
	...
	public static function request( $class, $function, $args ) {
		...
		try {
			...
			// Card declined
			return self::error_handler( 'card_error', esc_html__( 'Card Error', 'simple-pay' ) . ': ' . $e->getMessage() );

		} catch ( Error\RateLimit $e ) {
			// Too many requests made to the API too quickly
			return self::error_handler( 'rate_limit', esc_html__( 'Rate Limit Error', 'simple-pay' ) . ': ' . $e->getMessage() );

		} catch ( Error\InvalidRequest $e ) {
			// Invalid parameters were supplied to Stripe's API
			return self::error_handler( 'invalid_request', esc_html__( 'Invalid Request Error', 'simple-pay' ) . ': ' . $e->getMessage() );

		} catch ( Error\Authentication $e ) {
			// Authentication with Stripe's API failed
			// (maybe you changed API keys recently)
			return self::error_handler( 'authentication', esc_html__( 'Authentication Error', 'simple-pay' ) . ': ' . $e->getMessage() );

		} catch ( Error\ApiConnection $e ) {
			// Network communication with Stripe failed
			return self::error_handler( 'api_connection', esc_html__( 'Stripe API Connection Error', 'simple-pay' ) . ': ' . $e->getMessage() );

		} catch ( Error\Base $e ) {
			// Display a very generic error to the user, and maybe send
			// yourself an email
			return self::error_handler( 'generic', esc_html__( 'Stripe Error', 'simple-pay' ) . ': ' . $e->getMessage() );

		} catch ( \Exception $e ) {
			// Something else happened, completely unrelated to Stripe
			return self::error_handler( 'non_stripe', esc_html__( 'General Error', 'simple-pay' ) . ': ' . $e->getMessage() );
		}
	}
