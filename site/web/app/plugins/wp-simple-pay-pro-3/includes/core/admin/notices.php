...
class Notices {
	...
	public function api_keys_error() {
		...
		if ( ! simpay_check_keys_exist() && ( ( false !== $this->is_admin_screen && ( 'simpay_settings' === $this->is_admin_screen && isset( $_GET['tab'] ) && 'keys' !== $_GET['tab'] && 'license' !== $_GET['tab'] ) || 'simpay' === $this->is_admin_screen ) ) ) {
			...
			$notice_message = sprintf( __( 'Your %1$s Stripe API Keys for %2$s have not been entered.', 'simple-pay' ), ( simpay_is_test_mode() ? _x( 'test', 'Your test Stripe API keys...', 'simple-pay' ) : _x( 'live', 'Your live Stripe API keys...', 'simple-pay' ) ), SIMPLE_PAY_PLUGIN_NAME );
			$notice_message .= ' <a href="' . admin_url( 'admin.php?page=simpay_settings&tab=keys' ) . '">' . esc_html__( 'Enter them here.', 'simple-pay' ) . '</a>';
