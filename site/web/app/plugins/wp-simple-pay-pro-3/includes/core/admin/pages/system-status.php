	...
	private static function stripe_tls_check( $for_export ) {
		...
		try {
			...
			if ( $for_export ) {
				...
				return '<mark class="error">' . $retval . ' ' . sprintf( __( '<a href="%s" target="_blank">Please read this</a> for more information.', 'simple-pay' ), 'https://support.stripe.com/questions/how-do-i-upgrade-my-stripe-integration-from-tls-1-0-to-tls-1-2' ) . '</mark>';
			}
		}
	}
		...
		<div class="wrap">
			<div id="simpay-settings">
				<div id="simpay-settings-content">
					...
					if ( $memory < 41943040 ) {
						$memory = '<mark class="error">' . sprintf( __( '%1$s - It is recommendend to set memory to at least 40MB. See: <a href="%2$s" target="_blank">Increasing memory allocated to PHP</a>', 'simple-pay' ), $memory_export, 'http://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP' ) . '</mark>';
					} else {
					...
					if ( version_compare( PHP_VERSION, '7.0', '<' ) ) {
						$php = '<mark>' . PHP_VERSION . ' - ' . __( 'WordPress.org recommends upgrading to PHP 7 or higher for better security.', 'simple-pay' ) . ' <a href="https://wordpress.org/about/requirements/" target="_blank">' . __( 'Read more.', 'simple-pay' ) . '</a>' . '</mark>';
					} else {
